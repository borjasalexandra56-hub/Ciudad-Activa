...
export default function Messages() {
  ...
  const startNewConversation = async () => {
    ...
    setConversations(prev => [{ id: convId, lastMsg: { content: 'Nueva conversación', created_date: new Date().toISOString() }, messages: [] }, ...prev]);
    setActiveConvId(convId);
    setMessages([]);
  };
      ...
      <div className={`${activeConvId ? 'hidden lg:flex' : 'flex'} flex-col w-full lg:w-80 border-r border-border bg-card`}>
        <div className="p-4 border-b border-border">
          <div className="flex items-center justify-between mb-3">
            ...
            <Button size="sm" onClick={startNewConversation} className="rounded-xl bg-secondary hover:bg-secondary/90 text-white gap-1 h-8">
